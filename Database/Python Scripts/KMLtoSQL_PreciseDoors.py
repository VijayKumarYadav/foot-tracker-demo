...
conn = db.connect('AFloorNGB_doors.db')
cur = conn.cursor()
    ...
    for row in rows :
        ...
        for i in range(len(points)-1):
            ...
            if distance < min_dist:
                min_dist = distance
                wall = segment
        cur.execute("SELECT AsText(Buffer(doors.position, 0.00001)) FROM doors WHERE id = %s" % id)
        buffer = "GeomFromText('%s',4326)" % cur.fetchone()[0]
        print("UPDATE doors SET wall = Intersection(%s, %s) WHERE id = %s;" % (wall, buffer, id))
        cur.execute("UPDATE doors SET wall = Intersection(%s, %s) WHERE id = %s;" % (wall, buffer, id))
